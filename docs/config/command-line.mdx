...
<ParamField path="--transport" type="string" env="TRANSPORT" default="stdio">
  ...
  **Options:**
  ...
  npx dbhub-schema --transport stdio --dsn "..."

  # http - for web clients, workbench, and remote access
  npx dbhub-schema --transport http --port 8080 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--port" type="number" env="PORT" default="8080">
  ...
  ```bash
  npx dbhub-schema --transport http --port 3000 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--schema" type="string" env="SCHEMA">
  Default schema for this connection. (dbhub-schema fork.)
  ...
  ```bash
  npx dbhub-schema --dsn "postgres://user:pass@host:5432/mydb" --schema my_app_schema
  ```
...
<ParamField path="--id" type="string" env="ID">
  ...
  ```bash
  npx dbhub-schema --id prod --dsn "postgres://user:pass@prod-host:5432/db"
  npx dbhub-schema --id staging --dsn "postgres://user:pass@staging-host:5432/db"
  ```
...
<ParamField path="--demo" type="boolean" default="false">
  ...
  ```bash
  npx dbhub-schema --demo --transport http --port 8080
  ```
</ParamField>
...
<ParamField path="--config" type="string" default="./dbhub.toml">
  ...
  ```bash
  npx dbhub-schema --config ./dbhub.toml --transport http --port 8080
  ```
