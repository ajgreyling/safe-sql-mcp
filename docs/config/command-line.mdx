...
<ParamField path="--transport" type="string" env="TRANSPORT" default="stdio">
  ...
  **Options:**
  ...
  npx safe-sql-mcp --transport stdio --dsn "..."

  # http - for web clients, workbench, and remote access
  npx safe-sql-mcp --transport http --port 8080 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--port" type="number" env="PORT" default="8080">
  ...
  ```bash
  npx safe-sql-mcp --transport http --port 3000 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--schema" type="string" env="SCHEMA">
  Default schema for this connection. (safe-sql-mcp fork.)

  - **PostgreSQL**: Sets the connection `search_path` so `execute_sql` runs in that schema by default; unqualified table names resolve to this schema.
  - **All databases**: `search_objects` is restricted to this schema unless the tool is called with an explicit `schema` argument.

  ```bash
  npx safe-sql-mcp --dsn "postgres://user:pass@host:5432/mydb" --schema my_app_schema
  ```

  In TOML, set `schema = "my_app_schema"` under `[[sources]]` for the same behavior. See [TOML Configuration](/config/toml).
</ParamField>
...
<ParamField path="--id" type="string" env="ID">
  ...
  ```bash
  npx safe-sql-mcp --id prod --dsn "postgres://user:pass@prod-host:5432/db"
  npx safe-sql-mcp --id staging --dsn "postgres://user:pass@staging-host:5432/db"
  ```
...
<ParamField path="--demo" type="boolean" default="false">
  ...
  ```bash
  npx safe-sql-mcp --demo --transport http --port 8080
  ```
</ParamField>
...
<ParamField path="--config" type="string" default="./dbhub.toml">
  ...
  ```bash
  npx safe-sql-mcp --config ./dbhub.toml --transport http --port 8080
  ```
...
| `--schema` | `SCHEMA` | string | Default schema (PostgreSQL: search_path; all: filter search_objects) |
