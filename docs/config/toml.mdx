...
npx safe-sql-mcp --transport http --port 8080

# Or specify custom location
npx safe-sql-mcp --config=/path/to/config.toml
```
...
<ParamField path="schema" type="string">
  Default schema for this source. (safe-sql-mcp fork.) Same behavior as the [`--schema`](/config/command-line#--schema) command-line flag.

  - **PostgreSQL**: Sets the connection `search_path` so `execute_sql` runs in that schema by default.
