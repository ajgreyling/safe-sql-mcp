...
npx dbhub-schema --transport http --port 8080

# Or specify custom location
npx dbhub-schema --config=/path/to/config.toml
```
...
<ParamField path="schema" type="string">
  Default schema for this source. (dbhub-schema fork.) Same behavior as the [`--schema`](/config/command-line#--schema) command-line flag.
